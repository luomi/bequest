function assets_moments = Sim_Moments_par(parameter,e_states,a_grid,T,r,data)
...
parameter1  = parameter((n_type_var+1):(n_type_var+9));
parameter2  = parameter((n_type_var+10):(n_type_var+18));

parfor ii = 1:10
    if ii <= 5
    [~,P_sj_all{ii},~,~,~]...
        = EGM_Simplified(Final_age,ii,T,parameter1,gender_in,r,a_grid,e_states);
    elseif ii > 5
    [~,P_sj_all{ii},~,~,~]...
        = EGM_Simplified(Final_age,ii-5,T,parameter2,gender_in,r,a_grid,e_states);
    end
end
...
P_sj1_1     = P_sj_all{1,1};
P_sj2_1     = P_sj_all{2,1};
P_sj3_1     = P_sj_all{3,1};
P_sj4_1     = P_sj_all{4,1};
P_sj5_1     = P_sj_all{5,1};
P_sj1_2     = P_sj_all{6,1};
P_sj2_2     = P_sj_all{7,1};
P_sj3_2     = P_sj_all{8,1};
P_sj4_2     = P_sj_all{9,1};
P_sj5_2     = P_sj_all{10,1};


% CO_sj1_1    = CO_sj_all{1,1};
% CO_sj2_1    = CO_sj_all{2,1};
% CO_sj3_1    = CO_sj_all{3,1};
% CO_sj4_1    = CO_sj_all{4,1};
% CO_sj5_1    = CO_sj_all{5,1};
% CO_sj1_2    = CO_sj_all{6,1};
% CO_sj2_2    = CO_sj_all{7,1};
% CO_sj3_2    = CO_sj_all{8,1};
% CO_sj4_2    = CO_sj_all{9,1};
% CO_sj5_2    = CO_sj_all{10,1};
% 
% CF_sj1_1    = CF_sj_all{1,1};
% CF_sj2_1    = CF_sj_all{2,1};
% CF_sj3_1    = CF_sj_all{3,1};
% CF_sj4_1    = CF_sj_all{4,1};
% CF_sj5_1    = CF_sj_all{5,1};
% CF_sj1_2    = CF_sj_all{6,1};
% CF_sj2_2    = CF_sj_all{7,1};
% CF_sj3_2    = CF_sj_all{8,1};
% CF_sj4_2    = CF_sj_all{9,1};
% CF_sj5_2    = CF_sj_all{10,1};
...
Simulated_C     = cell(Ind,1);
Simulated_CO    = cell(Ind,1);
Simulated_CF    = cell(Ind,1);
...
psi_fam = [1/(1+exp(parameter(n_type_var+4)));1/(1+exp(parameter(n_type_var+13)))];
C_F     = [exp(parameter(n_type_var+7));exp(parameter(n_type_var+16))];
...
for i=1:Ind
    ...
    Simulated_C{i}(1:(start_age-1)) =0/0;
    Simulated_CO{i}(1:(start_age-1))=0/0;
    Simulated_CF{i}(1:(start_age-1))=0/0;
    ...
    for k=1:(deathtime(i)-1)
        ee      = need_path{i}(k);
        if pref_type(i) == 1
            kappa   = epsilon_grid1(ee);
        elseif pref_type(i) == 2
            kappa   = epsilon_grid2(ee);
        end
        inc_lvl = Income(start_age+54+k,gender(i),Inc_state(i));
        ...
        Simulated_C{i}(start_age+k-1)  = (1+r).*Simulated_Assets{i}(start_age+k-1)+inc_lvl-i_cost_path(k)-Simulated_Assets{i}(start_age+k);
        Simulated_CO{i}(start_age+k-1) = max([psi_fam(pref_type(i)).*(Simulated_C{i}(start_age+k-1)+kappa);C_F(pref_type(i))]);
        Simulated_CF{i}(start_age+k-1) = max([(1-psi_fam(pref_type(i))).*(Simulated_C{i}(start_age+k-1)+kappa) - kappa;0]);
        
        
        
                %%Check the removal of k+1 to k in the line above in the P_sj indexing.... This
                %%comes due to the fact that I recoded the model solution
                %%to ensure that the first policy function is in the first
                %%cell of the object.
    ...
    Simulated_Assets{i}(start_age+deathtime(i)-1)=[];
...
for io=1:Ind
    ...
    co_t{1}(io)    = Simulated_CO{io}(1);
    cf_t{1}(io)    = Simulated_CF{io}(1);
end
...
for k=2:period;
    ...
    for j=living_I'
        ...
        co_t{k}     = [co_t{k};Simulated_CO{j}(k)];
        cf_t{k}     = [cf_t{k};Simulated_CF{j}(k)];
    end
...
N_mom=12;
assets_mean_vec_3yr=zeros(N_mom,1);
assets_sd_vec_3yr=zeros(N_mom,1);
assets_per_25_vec_3yr=zeros(N_mom,1);
assets_per_50_vec_3yr=zeros(N_mom,1);
assets_per_75_vec_3yr=zeros(N_mom,1);

co_mean_vec_3yr=zeros(N_mom,1);
co_sd_vec_3yr=zeros(N_mom,1);
co_per_25_vec_3yr=zeros(N_mom,1);
co_per_50_vec_3yr=zeros(N_mom,1);
co_per_75_vec_3yr=zeros(N_mom,1);

cf_mean_vec_3yr=zeros(N_mom,1);
cf_sd_vec_3yr=zeros(N_mom,1);
cf_per_25_vec_3yr=zeros(N_mom,1);
cf_per_50_vec_3yr=zeros(N_mom,1);
cf_per_75_vec_3yr=zeros(N_mom,1);

for jj=1:N_mom
    assets_mean_vec_3yr(jj)=nanmean([assets_t{(jj-1)+1};assets_t{(jj-1)+2};assets_t{(jj-1)+3}])
    assets_sd_vec_3yr(jj)=nanstd([assets_t{(jj-1)+1};assets_t{(jj-1)+2};assets_t{(jj-1)+3}])
    assets_per_25_vec_3yr(jj)=prctile([assets_t{(jj-1)+1};assets_t{(jj-1)+2};assets_t{(jj-1)+3}],25)
    assets_per_50_vec_3yr(jj)=prctile([assets_t{(jj-1)+1};assets_t{(jj-1)+2};assets_t{(jj-1)+3}],50)
    assets_per_75_vec_3yr(jj)=prctile([assets_t{(jj-1)+1};assets_t{(jj-1)+2};assets_t{(jj-1)+3}],75)
    
    co_mean_vec_3yr(jj)=nanmean([co_t{(jj-1)+1};co_t{(jj-1)+2};co_t{(jj-1)+3}])
    co_sd_vec_3yr(jj)=nanstd([co_t{(jj-1)+1};co_t{(jj-1)+2};co_t{(jj-1)+3}])
    co_per_25_vec_3yr(jj)=prctile([co_t{(jj-1)+1};co_t{(jj-1)+2};co_t{(jj-1)+3}],25)
    co_per_50_vec_3yr(jj)=prctile([co_t{(jj-1)+1};co_t{(jj-1)+2};co_t{(jj-1)+3}],50)
    co_per_75_vec_3yr(jj)=prctile([co_t{(jj-1)+1};co_t{(jj-1)+2};co_t{(jj-1)+3}],75)
    
    cf_mean_vec_3yr(jj)=nanmean([cf_t{(jj-1)+1};cf_t{(jj-1)+2};cf_t{(jj-1)+3}])
    cf_sd_vec_3yr(jj)=nanstd([cf_t{(jj-1)+1};cf_t{(jj-1)+2};cf_t{(jj-1)+3}])
    cf_per_25_vec_3yr(jj)=prctile([cf_t{(jj-1)+1};cf_t{(jj-1)+2};cf_t{(jj-1)+3}],25)
    cf_per_50_vec_3yr(jj)=prctile([cf_t{(jj-1)+1};cf_t{(jj-1)+2};cf_t{(jj-1)+3}],50)
    cf_per_75_vec_3yr(jj)=prctile([cf_t{(jj-1)+1};cf_t{(jj-1)+2};cf_t{(jj-1)+3}],75)    
end

assets_moments  = [assets_per_25_vec_3yr; assets_per_50_vec_3yr; assets_per_75_vec_3yr]
co_moments      = [co_per_25_vec_3yr; co_per_50_vec_3yr; co_per_75_vec_3yr]
cf_moments      = [cf_per_25_vec_3yr; cf_per_50_vec_3yr; cf_per_75_vec_3yr]

save results.mat assets_mean_vec_3yr assets_sd_vec_3yr assets_moments ...
    co_mean_vec_3yr co_sd_vec_3yr co_moments cf_mean_vec_3yr cf_sd_vec_3yr cf_moments;
